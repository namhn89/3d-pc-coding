...
def split_octree(points, bbox_min, bbox_max):
    ...
    returns points in local coordinates (block) and
    octree structure as an 8 bit integer (right to left order)

    Args:
        points ([type]): [description]
        bbox_min ([type]): [description]
        bbox_max ([type]): [description]
    ...
    for point in points:
        ...
        if point[2] >= midpoint[2]:
            location |= 0b100
        ret_points[location].append(
            point - np.pad(global_bboxes[location][0], [0, len(point) - 3])
        )
    binstr = 0b00000000
    ...
    return (
        [np.vstack(rp) for rp in ret_points if len(rp) > 0],
        binstr,
        local_bboxes,
    )
...
def partition_octree_rec(points, bbox_min, bbox_max, level):
    ...
    result = [
        partition_octree(rp, bbox[0], bbox[1], level - 1)
        for rp, bbox in zip(ret_points, bboxes)
    ]
    blocks = [
        subblock
        for block_res in result
        for subblock in block_res[0]
        if len(subblock) > 0
    ]
    new_binstr = [binstr] + [
        subbinstr
...
def partition_octree(points, bbox_min, bbox_max, level):
    ...
    assert (
        geo_level >= level
    )  # check whether geo level larger than level, if not, raise an error
    block_size = 2 ** (
        geo_level - level
    )  # high level value --> smaller block size

    # Compute partitions for each point
    block_ids = (
        points[:, :3] // block_size
    )  # floor division, like quantization
    ...
    for x, y, z in block_ids_unique:
        zip_params = [
            f"{v:0{geo_level - level}b}" for v in [z, y, x]
        ]  # output: 1008b, 1308b... 10, 13 are z and y,
        sort_key.append("".join(i + j + k for i, j, k in zip(*zip_params)))
    sort_idx = np.argsort(
        sort_key
    )  # return index of value sorted array to reconstruct original
    ...
    local_refs = np.pad(
        block_ids_unique[block_idx] * block_size,
        [[0, 0], [0, points.shape[1] - 3]],
    )  # add padding in block ids unique, to have the same size as points
    points_local = (
        points - local_refs
    )  # rotate,translate original coordinatte to box coordinate by subtract the reference

    # Group points by block
    blocks = [
        np.zeros((_len, points.shape[_len])) for _len in block_len
    ]  # blocks: l in block len:number of point are assign to the same block
    ...
    for i, b_idx in enumerate(block_idx):
        blocks[b_idx][blocks_last_idx[b_idx]] = points_local[
            i
        ]  # store point coordinate in blocks
    ...
    _, binstr = partition_octree_rec(
        block_ids_unique, [0, 0, 0], (2 ** level) * np.array([1, 1, 1]), level
    )
        ...
        while binstr_list[binstr_list_idx] != 0 and not child_found:
            if (binstr_list[binstr_list_idx] & 1) == 1:
                ...
                if cur_level == level:
                    ...
                    blocks[block_idx] = blocks[block_idx] + np.pad(
                        cur_bbox[0], [0, blocks[block_idx].shape[1] - 3]
                    )
