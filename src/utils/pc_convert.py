...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--level", type=int, help="Octree decomposition level.", default=3
    )
    parser.add_argument(
        "--source_extension", help="Mesh files extension", default=".ply"
    )
    parser.add_argument(
        "--target_extension", help="Point cloud extension", default=".ply"
    )
    args = parser.parse_args()

    assert os.path.exists(args.source), print(
        "{} does not exist".format(args.source)
    )
    assert args.vg_size > 0, print("vg_size must be positive")

    paths = glob(
        join(args.source, "**", f"*{args.source_extension}"), recursive=True
    )
