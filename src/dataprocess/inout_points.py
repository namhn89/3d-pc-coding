...
def load_points(filename, cube_size=64, min_num=20):
    ...
    for _, point in enumerate(point_cloud):
        ...
        if cube_index not in cubes.keys():
    ...
    cube_positions_n = (
        cube_positions[:, 0:1]
        + cube_positions[:, 1:2] * step
        + cube_positions[:, 2:3] * step * step
    )
...
def voxels2points(voxels):
    ...
    set_points = []
    for _, vol in enumerate(voxels):  # vol.shape: (64, 64, 64)
        points = np.array(np.where(vol > 0)).transpose(
            (1, 0)
        )  # points.shape: (1266, 3)
...
def select_voxels(vols, points_nums, offset_ratio=1.0, fixed_thres=None):
    ...
    input:  vols: [batch_size, 1, vsize, vsize, vsize] float32
            points numbers: [batch_size]
    output: the mask (0 or 1) representing the selected voxels: [batch_size, 1, vsize, vsize, vsize]
    """
    # vols = vols.numpy()
